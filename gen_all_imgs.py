...
# Examples
examples = [
    (0.8,2.4,'苯 C6H6','DBE=(12+2-6)/2=4\n(1环+3双键)','#FF5722'),
    (3.3,2.4,'环己烷 C6H12','DBE=(12+2-12)/2=1\n(1个环)','#4CAF50'),
    (5.8,2.4,'乙炔 C2H2','DBE=(4+2-2)/2=2\n(1个三键=2)','#9C27B0'),
    (8.3,2.4,'丙酮 C3H6O','DBE=(6+2-6)/2=1\n(1个C=O)','#FF9800'),
]
...
ax1.plot([1.5,0.5],[2,3],'b-',lw=2); ax1.text(0.3,3.2,'CH3',fontsize=11,color='blue',fontweight='bold')
ax1.plot([1.5,0.5],[2,1],'r-',lw=2); ax1.text(0.3,0.7,'Br',fontsize=11,color='red',fontweight='bold')
# Right carbon substituents
ax1.plot([3.5,4.5],[2,3],'g-',lw=2); ax1.text(4.5,3.2,'CH3',fontsize=11,color='green',fontweight='bold')
ax1.plot([3.5,4.5],[2,1],'orange',lw=2); ax1.text(4.5,0.7,'H',fontsize=11,color='orange',fontweight='bold')
ax1.text(1.5,2.6,'(1)Br>CH3',fontsize=9,ha='center',color='purple')
ax1.text(3.5,2.6,'(1)CH3>H',fontsize=9,ha='center',color='purple')
...
ax2.plot([1.5,0.5],[2,3],'b-',lw=2); ax2.text(0.3,3.2,'CH3',fontsize=11,color='blue',fontweight='bold')
ax2.plot([1.5,0.5],[2,1],'gray',lw=2); ax2.text(0.3,0.7,'H',fontsize=11,color='gray')
ax2.plot([3.5,4.5],[2,3],'b-',lw=2); ax2.text(4.5,3.2,'CH3',fontsize=11,color='blue',fontweight='bold')
...
rules = [
    ('(1)','原子骨架不变','只移动电子，不移动原子','#1565C0'),
    ('(2)','价电子总数不变','总电子数守恒','#1565C0'),
    ('(3)','只移动π电子和孤对电子','σ键电子不参与共振','#C62828'),
    ('(4)','每个原子不超过8电子','第二周期元素！(H除外最多2)','#C62828'),
    ('(5)','电荷分布尽量少','贡献大的结构更重要','#2E7D32'),
    ('(6)','负电荷在电负性大的原子上','O,N上的负电荷比C稳定','#2E7D32'),
    ('(7)','等价共振结构贡献相同','如苯的两个Kekulé结构','#6A1B9A'),
]
...
cl_data = ['Cl2','光照/高温','低','1','4','5','工业卤化']
br_data = ['Br2','光照(较慢)','高','1','80','1600','合成选择性']
# Draw table
...
ax.text(0.5,1.55,'(1)引发: Cl2 →(hν) 2Cl·  (均裂)',fontsize=9,family='monospace')
ax.text(0.5,1.15,'(2)增长: Cl· + R-H → R· + HCl    R· + Cl2 → R-Cl + Cl·',fontsize=9,family='monospace')
ax.text(0.5,0.75,'(3)终止: 2R· → R-R   R·+Cl·→ R-Cl   2Cl·→Cl2',fontsize=9,family='monospace')
...
ax.text(5.5,0.3,'=> Br2选择性更好，多用于合成',fontsize=9,color='green',fontweight='bold')
...
draw_chair(axes[0],'甲基环己烷：e键构象\n（平伏键）',
    2,'CH3','#2196F3','更稳定 ΔG=-7.3 kJ/mol')
...
ax2.plot([cx,cx],[cy,cy+1.2],'b-',lw=2.5); ax2.text(cx,cy+1.4,'(1)',ha='center',fontsize=12,color='blue',fontweight='bold')
ax2.plot([cx,cx-1.1],[cy,cy-0.7],'b-',lw=2.5); ax2.text(cx-1.3,cy-0.9,'(2)',ha='center',fontsize=12,color='blue',fontweight='bold')
ax2.plot([cx,cx+1.1],[cy,cy-0.7],'b-',lw=2.5); ax2.text(cx+1.3,cy-0.9,'(3)',ha='center',fontsize=12,color='blue',fontweight='bold')
ax2.plot([cx,cx],[cy,cy-1.4],'r--',lw=2.5); ax2.text(cx,cy-1.7,'(4)(H)\n← 指向里',ha='center',fontsize=10,color='red')
ax2.text(2,0.3,'最低优先基((4))必须\n朝向观察者背面',ha='center',fontsize=9.5,style='italic',color='green')

ax3.set_title('(3)观察(1)→(2)→(3)转向\n判断R/S', fontsize=11, fontweight='bold', color='#C62828')
...
ax3.text(2,0.3,'(4)在前时：R<->S互换！',ha='center',fontsize=9.5,color='purple',style='italic')
...
ax.text(6,0.35,'含n个不同手性中心→最多2n种立体异构体；有meso化合物时减少',
    ha='center',fontsize=10,color='gray')
# Arrows
...
# Example: propene + HBr
ax.text(0.5,3.8,'CH3-CH=CH2  +  HBr', fontsize=12, family='monospace')
ax.text(5.5,3.8,'→', fontsize=16)
# Two pathways
ax.text(3.5,3.0,'途径A（Markovnikov）:', fontsize=10, color='green', fontweight='bold')
ax.text(3.5,2.5,'H+→CH2=  形成 CH3-CH+-CH3', fontsize=9.5, family='monospace', color='green')
ax.text(3.5,2.0,'2°碳正离子 → 稳定 → 主产物', fontsize=9.5, color='green')
ax.text(3.5,1.5,'→ CH3-CHBr-CH3  (2-溴丙烷)', fontsize=9.5, family='monospace', color='green', fontweight='bold')

ax.text(7.5,3.0,'途径B（非Markovnikov）:', fontsize=10, color='red', fontweight='bold')
ax.text(7.5,2.5,'H+→CH=  形成 CH3-CH=CH2→CH3CH2CH2+', fontsize=9, family='monospace', color='red')
ax.text(7.5,2.0,'1°碳正离子 → 不稳定 → 次产物', fontsize=9.5, color='red')
ax.text(7.5,1.5,'→ CH3-CH2-CH2Br  (1-溴丙烷)', fontsize=9.5, family='monospace', color='red')
...
ax.text(6,0.35,'记忆口诀：冷稀/OsO4→顺二醇  热浓→断键  O3→切断  mCPBA→环氧',
...
fig, ax = plt.subplots(figsize=(11,4.5))
ax.axis('off'); ax.set_xlim(0,11); ax.set_ylim(0,4.5)
ax.set_title('硼氢化-氧化反应（BH3-THF → H2O2/NaOH）', fontsize=13, fontweight='bold')

# Boxes
b1=mpatches.FancyBboxPatch((0.3,0.8),2.5,2.8,boxstyle='round,pad=0.1',
    facecolor='#E3F2FD',edgecolor='#1565C0',lw=2)
ax.add_patch(b1)
ax.text(1.55,3.3,'步骤(1)：硼氢化',fontsize=10,fontweight='bold',color='#1565C0',ha='center')
ax.text(1.55,2.7,'BH3与C=C协同加成\n(四中心过渡态)',fontsize=9,ha='center')
...
ax.text(5.5,0.3,'BH3加成 vs 酸催化水合：结果互补！酸催化→Markovnikov醇；BH3→反Markovnikov醇',
...
ax.text(0.5,3.3,'  (CH3)2C+-CH2CH3  →  (CH3)2C=CH-CH3 ?',fontsize=9,family='monospace')
...
ax.text(0.5,1.5,'  (CH3)3C-CH2+ → (CH3)2C+-CH2CH3（经过重排）',fontsize=9,color='green',family='monospace')
...
b1=mpatches.FancyBboxPatch((0.3,0.8),4.8,3.5,boxstyle='round,pad=0.1',
    facecolor='#E3F2FD',edgecolor='#1565C0',lw=2)
ax.add_patch(b1)
ax.text(2.7,4.0,'Lindlar催化剂 + H2',fontsize=11,fontweight='bold',ha='center',color='#1565C0')
ax.text(2.7,3.4,'Pd/BaSO4 + 喹啉\n（毒化催化剂）',fontsize=9.5,ha='center')
...
ax.text(2.7,2.4,'活化能低→更快生成\n1,2-加成产物占优\n（BrCH2更好亲核）',fontsize=9.5,ha='center')
...
# Terminal alkyne
ax.text(0.5,4.0,'(1)端炔水合（HgSO4/H2SO4/H2O）：',fontsize=10,fontweight='bold',color='#1565C0')
ax.text(0.5,3.4,'RC≡CH  →[H2O/Hg2+]→ RC(OH)=CH2 → RC(=O)-CH3',fontsize=10,family='monospace',color='#1565C0')
...
ax.text(0.5,1.6,'(2)乙炔水合（特殊）：',fontsize=10,fontweight='bold',color='#C62828')
ax.text(0.5,1.1,'HC≡CH  →[H2O/Hg2+]→ CH2=CHOH → CH3CHO',fontsize=10,family='monospace',color='#C62828')
...
huckel_data = [
    (1.2, 4.0, '苯 C6H6', 'n=1, 6π', '芳香\n(平面,环形共轭)', '#4CAF50', '✓'),
    (3.5, 4.0, '萘 C10H8', 'n=2, 10π', '芳香\n(稠环)', '#4CAF50', '✓'),
    (5.8, 4.0, '环戊二烯阴离子\nC5H5-', 'n=1, 6π', '芳香！\n(加H+pKa~15)', '#4CAF50', '✓'),
    (8.1, 4.0, '环庚三烯阳离子\nC7H7+(卓鎓)', 'n=1, 6π', '芳香！\n(特别稳定)', '#4CAF50', '✓'),
    (10.4,4.0, '吡啶 C5H5N', 'n=1, 6π\n(N提供1对孤电子)', '芳香\n(弱碱)', '#4CAF50', '✓'),
    (1.2, 1.8, '环丁二烯\nC4H4', 'n=1, 4π', '反芳香！\n(不稳定)', '#F44336', '✗'),
    (3.5, 1.8, '环辛四烯\nC8H8', '8π e-\n(4n,n=2)', '非芳香\n(盆形,非平面)', '#FF9800', '△'),
    (5.8, 1.8, '吡咯 C4H4NH', 'n=1, 6π\n(N孤对加入)', '芳香！\n(pKa~17,弱酸)', '#4CAF50', '✓'),
    (8.1, 1.8, '呋喃 C4H4O', 'n=1, 6π\n(O孤对1对加入)', '芳香\n(弱)', '#4CAF50', '✓'),
    (10.4,1.8, '噻吩 C4H4S', 'n=1, 6π\n(S孤对)', '芳香\n(较好)', '#4CAF50', '✓'),
]
...
ax.text(6,0.4,'芳香性必要条件：(1)平面 (2)环状 (3)全共轭 (4)4n+2个π电子',
    ha='center',fontsize=11,style='italic',color='purple',
    bbox=dict(facecolor='lightyellow',boxstyle='round'))
...
ax.text(3.1,4.3,'—NH2, —NHR, —NR2（强活化）',fontsize=9.5,ha='center')
ax.text(3.1,3.9,'—CH3, —R（弱活化）',fontsize=9.5,ha='center')
...
ax.text(9.9,4.7,'—NO2（强钝化）',fontsize=9.5,ha='center')
...
ax.text(9.9,3.1,'—SO3H（钝化）',fontsize=9.5,ha='center')
ax.text(9.9,2.7,'—CF3, —CCl3（钝化）',fontsize=9.5,ha='center')
...
eas_reactions = [
    (1.3,4.8,'卤化','ArH + Cl2/Br2\n→(FeCl3/FeBr3)\nArX + HX',
     '亲电试剂: Cl+(ClFeCl3-)\n路易斯酸活化卤素\n不需要光（区别于自由基）','#1565C0'),
    (4.0,4.8,'硝化','ArH + HNO3\n→(浓H2SO4)\nArNO2 + H2O',
     '亲电试剂: NO2+(硝酰离子)\nH2SO4质子化HNO3\n温度控制多取代','#C62828'),
    (6.7,4.8,'磺化','ArH + H2SO4(发烟)\n→ ArSO3H',
     '亲电试剂: SO3\n可逆反应！\n稀H2SO4高温→去磺化','#2E7D32'),
...
ax.text(5.5,1.0,'区分：环上反应需Lewis酸（FeBr3等）\n侧链卤代需光照（自由基）',
...
ax.text(0.5,4.0,'[!] 无水！无CO2！避免水和质子性物质',fontsize=10,color='red',fontweight='bold')

# Reactions
grignard_rxns = [
    (1.5,2.8,'+ 甲醛 (HCHO)','→ 伯醇 (RCH2OH)','#1565C0'),
    (1.5,2.1,'+ 醛 (RCHO)','→ 仲醇 (RCHOHR\')\n增加一个碳','#2196F3'),
    (1.5,1.4,'+ 酮 (R\'COR\'\')',  '→ 叔醇 (R\'R\'\'COHR)\n增加一个碳','#42A5F5'),
    (6.5,2.8,'+ CO2','→ (RCOOMgX) → H3O+ → RCOOH\n增加一个碳→羧酸','#C62828'),
    (6.5,2.1,'+ 环氧乙烷','→ RCH2CH2OH\n增加两个碳','#E53935'),
    (6.5,1.4,'+ H2O/醇/酸\n(含活泼H)',  '→ R-H\n(烃)\n破坏Grignard！','#FF9800'),
]
...
ax.text(5,0.6,'【活性要求】\n二烯体：必须s-cis构象 + 富电子（供电子基）更活泼\n亲二烯体：缺电子（吸电子基EWG：CHO,NO2,CN,C=O）更活泼',
    ha='center',fontsize=9,style='italic',color='gray',
    bbox=dict(facecolor='lightyellow',boxstyle='round'))
